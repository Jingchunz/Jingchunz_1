...
2. Clone your Github repository on your local machine
    ...
    ```bash
    $ git clone <<git clone <<https://github.com/Jingchunz/Jingchunz_1.git>>
    $ cd hw-1
    ```
...
count_positive <- sum(my_vec_double > 0)
...
I expect row_wise_scan will be faster becasuse scan by rows means frequently jumping between different columns, which is more efficient in terms of memory access.
...
for (size in sizes) {
    ...
    row_time <- time_scan(row_wise_scan, M)
    col_time <- time_scan(col_wise_scan, M)
    times_list[[as.character(size)]] <- list(
        row_wise_time = row_time,
        col_wise_time = col_time
    )
}
...
```
The output indicates that column-wise scanning (col_wise_scan) is consistently faster than row-wise scanning (row_wise_scan) across different matrix sizes. The performance difference becomes more significant as matrix size increases, confirming that R's column-major storage order favors column-wise operations, especially for large matrices. For a $5000 \times 5000$ matrix, the column-wise scan is notably quicker, taking about 3.151 seconds versus 5.502 seconds for the row-wise scan.
